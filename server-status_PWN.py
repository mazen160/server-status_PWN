...
parser.add_argument("-u","--url", dest="url",help="The Apache server-status URL.", action='store',required=True)
parser.add_argument("--sleeping-time", dest="sleeping_time",help="Sleeping time between each request. (Default: 10)", action='store',default=10)
parser.add_argument("--db", dest="db",help="Outputs database path. (Default: /tmp/server-status_PWN.db).",default='/tmp/server-status_PWN.db', action='store')
parser.add_argument("-o","--output", dest="output_path",help="Saves output constantly into a newline-delimited output file.", action='store')
parser.add_argument("--enable-full-logging", dest="enable_full_logging",help="Enable full logging for all requests with timestamps of each request.", default=False, action='store_true')
...
class Request_Handler():
	...
	def __init__(self):
		...
		self.additional_headers = {} # Fill this dict with additional headers if needed. # It will work normally by default.
	def send_request(self, url):
		...
		'''
		# I will leave this evil trick for you.
		# Uncomment these lines to activate it.
		conf_breaking_headers = {"X-Originating-IP": self.origin_ip, "X-Forwarded-For": self.origin_ip, "X-Remote-IP": self.origin_ip, "X-Remote-Addr": self.origin_ip}
		...
		try:
			try:
				...
				pass
			req = requests.get(url,headers=headers,timeout=int(self.timeout), verify=False, allow_redirects=False)
	...
	while True:
		...
		if validate_output != True:
			...
			for _ in range(len(parsed_output['FULL_URL'])):
				if ( DBHandler().Check_If_URL_Exists(parsed_output['FULL_URL'][_]) == False ) and ( parsed_output['FULL_URL'][_] != ''):
					try:
			...
			for _ in range(len(parsed_output['CLIENT_IP_ADDRESS'])):
				if ( DBHandler().Check_If_Client_Exists(parsed_output['CLIENT_IP_ADDRESS'][_]) == False):
